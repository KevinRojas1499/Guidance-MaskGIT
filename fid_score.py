...
def ensure_process_group_initialized(backend='nccl'):
    if not dist.is_initialized():
        # If not initialized, initialize the process group
...
def cli(path, ref_path, batch_size, num_workers, save_path, res):
    calculate_fid_given_paths(path, ref_path,  save_path, res, batch_size, num_workers)



def calculate_fid_given_paths(path, ref_path, save_path, res, batch_size=50, num_workers=4):
